 ...
 * @author Gunging
 */
public class ISPSStandard extends ISPSpigotIndexedStatement {
    /**
     ...
     * @author Gunging
     */
    public ISPSStandard(@NotNull ISPSpigotStatements statements, @NotNull GOOBWrapperBase<? extends GBMPlayer<?>,?> wrapper, @NotNull GBMNamespacedKey<?> name) {
     ...
     * @author Gunging
     */
    public ISPSStandard(@NotNull ISPSpigotStatements statements, @NotNull GOOBWrapperBase<? extends GBMPlayer<?>,?> wrapper, @NotNull GBMNamespacedKey<?> name, int slot) {
     ...
     * @author Gunging
     */
    public ISPSStandard(@NotNull ISPSpigotStatements statements, @NotNull GOOBWrapperBase<? extends GBMPlayer<?>,?> wrapper, @NotNull GBMNamespacedKey<?> name, @NotNull IntegerNumberRange qnr) {
     ...
     * @author Gunging
     */
    public ISPSStandard(@NotNull ISPSpigotStatements statements, @NotNull GOOBWrapperBase<? extends GBMPlayer<?>,?> wrapper, @NotNull GBMNamespacedKey<?> name, int minSlot, int maxSlot) {
     ...
     * @since 1.0.0
     */
    @Override public int getMaximumRange(@NotNull GBMPlayer<?> target) { return target.getInventory().getSize(); }

    /**
     * @author Gunging
     * @since 1.0.0
     */
    @Override public ISPSStandard of(@NotNull IntegerNumberRange qnr) {
        ISPSStandard crafting = new ISPSStandard(getStatements(), (GOOBWrapperBase<? extends GBMPlayer<?>,?>) getElaboratorWrapper(), getStatementName(), qnr);
    ...
    @Override public @Nullable GBMItemStack<?> readItemStack(@NotNull GBMPlayer<?> target) {
        ...
        // Return that slot
        return target.getInventory().getItem(min);
    }
    ...
    @Override public void writeItemStack(@NotNull GBMPlayer<?> target, @Nullable GBMItemStack<?> item) {
        ...
        if (item == null) {
            target.getInventory().setItem(min, getStatements().getMinecraft().getItemStacks().empty());
        } else {
            target.getInventory().setItem(min, item);
        }
    }
}
