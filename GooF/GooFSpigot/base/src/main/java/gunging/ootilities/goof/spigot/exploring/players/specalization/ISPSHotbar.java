 ...
 * @since 1.0.0
 */
public class ISPSHotbar extends ISPSpigotStatement {
     ...
     * @since 1.0.0
     */
    public ISPSHotbar(@NotNull ISPSpigotStatements statements, @NotNull GOOBWrapperBase<? extends GBMPlayer<?>,?> wrapper, @NotNull GBMNamespacedKey<?> statementName) {
     ...
     * @since 1.0.0
     */
    @Override public @NotNull ISPSHotbar withOptions(@NotNull String options) { return getStatements().HOTBAR; }
    ...
    @Override public @NotNull ArrayList<ItemStackExplorer<GBMPlayer<?>>> whenElaborated(@NotNull ItemExplorerElaborator<? extends GBMPlayer<?>> elaborator) {
        return new ArrayList<>(new ISPPlayerExplorer(getStatements().STANDARD.of(0, 8)).elaborate(elaborator));
    }
