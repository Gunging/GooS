package gunging.ootilities.goob.engine.minecraft.wrapping;

import gunging.ootilities.goob.engine.minecraft.GOOBMinecraft;
import gunging.ootilities.goob.engine.minecraft.GOOBWrapperBase;
import org.jetbrains.annotations.NotNull;

/**
 * The platform-dependent interface for Entities.
 *
 * @param <GooF> The wrapper class for Entities
 * @param <E> The Entity class of this platform
 *
 * @author Gunging
 * @since 1.0.0
 */
public abstract class GBMEntityWrapper<GooF extends GBMEntity<E>, E> extends GOOBWrapperBase<GooF, E> {

    /**
     * @param minecraft The engine this runs in
     *
     * @author Gunging
     * @since 1.0.0
     */
    public GBMEntityWrapper(@NotNull GOOBMinecraft minecraft) { super(minecraft); }
}
